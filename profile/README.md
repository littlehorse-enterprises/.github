...
1. [lh-developer-hub](https://github.com/littlehorse-enterprises/lh-developer-hub) contains examples, agent skills, and quickstarts. Everything you need to become a Business-as-Code expert. **This is probably the only repository you need to download to use LittleHorse well.**
2. [Documentation](https://littlehorse.io/docs) is available on our website.
3. [Quickstarts](https://littlehorse.io/docs/getting-started/quickstart) are available for all components of the LittleHorse platform.
...
1. [littlehorse](https://github.com/littlehorse-enterprises/littlehorse) contains the code for the LittleHorse Server, which is the core of our Business-as-Code platform.
2. [lh-agent-connector](https://github.com/littlehorse-enterprises/lh-agent-connector) contains a pre-built Task Worker that uses LangChain to implement the [Decision Worker Pattern](https://littlehorse.io/blog/decision-workers).
3. [lh-quarkus](https://github.com/littlehorse-enterprises/lh-quarkus) contains Quarkus extensions to make it really easy to write LittleHorse applications in Java.
4. [lh-kafka-connect](https://github.com/littlehorse-enterprises/lh-kafka-connect) contains Connecors for Apache Kafka Connect to start `WfRun`s (trigger workflows) and post `ExternalEvent`s in the LittleHorse Server.
5. [lh-user-tasks-bridge](https://github.com/littlehorse-enterprises/lh-user-tasks-bridge) connects your OIDC provider to the LittleHorse Server's [User Tasks](https://littlehorse.io/docs/server/concepts/user-tasks) capability, mapping user identities to assigned tasks.
